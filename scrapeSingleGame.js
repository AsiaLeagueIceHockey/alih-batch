...
async function scrapeGame(gameNoToScrape, scheduleData) {
  ...
  const isFinished = Boolean(game_info.game_time.end);

  return { home_score, away_score, isFinished };
}
...
async function main() {
  ...
  try {
    ...
    if (options.gameNos && options.gameNos.length > 0) {
      scheduleQuery = scheduleQuery.in('game_no', options.gameNos);
    } else {
      scheduleQuery = scheduleQuery
        .gt('match_at', fortyEightHoursAgo.toISOString())
        .lt('match_at', now.toISOString());
    }

    const { data: candidateGames, error: scheduleError } = await scheduleQuery;
    ...
    let gamesToScrape = candidateGames;
    ...
    if (gamesToScrape.length === 0) {
      console.log('No games require scraping. Exiting.');
      process.exit(0);
    }
        ...
        if (home_score !== null && away_score !== null) {
          const { error: updateError } = await supabase
            ...
            .update({ 
              ...
              ...(isFinished ? { game_status: 'Game Finished' } : {}),
            })
